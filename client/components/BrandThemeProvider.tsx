...
export function BrandThemeProvider() {
  ...
  useEffect(() => {
    const [primaryH, primaryS, primaryL] = hexToHsl(activeBrand.primaryColor);
    const [secondaryH, secondaryS, secondaryL] = hexToHsl(
    ...
    const existingGoldSpellStyle = document.getElementById(
      "gold-spell-gradient",
    );
    const existingExtremeGoldStyle = document.getElementById(
      "extreme-gold-sonner",
    );
    const existingEverkStyle = document.getElementById("everk-styles");
    if (existingGoldSpellStyle) existingGoldSpellStyle.remove();
    if (existingExtremeGoldStyle) existingExtremeGoldStyle.remove();
    if (existingEverkStyle) existingEverkStyle.remove();

    // Aplicar estilos específicos por marca
    if (activeBrand.name === "Gold Spell Cosméticos") {
      // Aplicar gradiente dourado específico
      const style = document.createElement("style");
      style.id = "gold-spell-gradient";

      style.textContent = `
        .dynamic-gradient {
          background: linear-gradient(90deg, #ae0223 0%, ${activeBrand.primaryColor} 100%);
        }

        @supports (color: color-mix(in lab, red, red)) {
          .dynamic-gradient {
            background: linear-gradient(90deg, color-mix(in srgb, ${activeBrand.primaryColor} 80%, black) 0%, ${activeBrand.primaryColor} 100%);
          }
        }
      `;

      document.head.appendChild(style);
    } else if (activeBrand.name === "Extreme Gold") {
      // Aplicar estilos completos Sonner Toast e animações para Extreme Gold
      const style = document.createElement("style");
      style.id = "extreme-gold-sonner";

      style.textContent = `
        /* Complete Sonner Toast System for Extreme Gold */
        ...
        .sonner-loading-bar:nth-child(1){animation-delay:-1.2s;transform:rotate(.0001deg) translate(146%)}
        ...
        .sonner-loading-bar:nth-child(4){animation-delay:-.9s;transform:rotate(90deg) translate(146%)}
        .sonner-loading-bar:nth-child(5){animation-delay:-.8s;transform:rotate(120deg) translate(146%)}
        .sonner-loading-bar:nth-child(6){animation-delay:-.7s;transform:rotate(150deg) translate(146%)}
        .sonner-loading-bar:nth-child(7){animation-delay:-.6s;transform:rotate(180deg) translate(146%)}
        .sonner-loading-bar:nth-child(8){animation-delay:-.5s;transform:rotate(210deg) translate(146%)}
        .sonner-loading-bar:nth-child(9){animation-delay:-.4s;transform:rotate(240deg) translate(146%)}
        .sonner-loading-bar:nth-child(10){animation-delay:-.3s;transform:rotate(270deg) translate(146%)}
        .sonner-loading-bar:nth-child(11){animation-delay:-.2s;transform:rotate(300deg) translate(146%)}
        .sonner-loading-bar:nth-child(12){animation-delay:-.1s;transform:rotate(330deg) translate(146%)}
        ...
        @keyframes sonner-spin{0%{opacity:1}to{opacity:.15}}

        /* Custom Extreme Gold Animations */
        ...
        @keyframes go901347462{from{transform:scale(0.6);opacity:0.4;}to{transform:scale(1);opacity:1;}}

        .go4109123758{z-index:9999;}
        .go4109123758 > *{pointer-events:auto;}

        /* Extreme Gold Brand Theme */
        .dynamic-gradient {
          background: linear-gradient(90deg, #3A9CA8 0%, ${activeBrand.primaryColor} 100%);
        }

        @supports (color: color-mix(in lab, red, red)) {
          .dynamic-gradient {
            background: linear-gradient(90deg, color-mix(in srgb, ${activeBrand.primaryColor} 80%, #3A9CA8) 0%, ${activeBrand.primaryColor} 100%);
          }
        }

        /* Apply Extreme Gold theme to elements */
        .border-primarycolor {
          border-color: ${activeBrand.primaryColor};
          animation: go901347462 0.3s ease-out;
        }

        .bg-secondarycolor {
          background-color: ${activeBrand.secondaryColor};
          animation: go3020080000 0.3s ease-out;
        }

        .text-primarycolor {
          color: ${activeBrand.primaryColor};
          animation: go2264125279 0.3s ease-out;
        }

        .bg-primarycolor {
          background-color: ${activeBrand.primaryColor};
          animation: go463499852 0.3s ease-out;
        }

        /* Enhanced toast notifications for Extreme Gold */
        [data-sonner-toast] {
          border-color: ${activeBrand.primaryColor} !important;
          background: linear-gradient(135deg, ${activeBrand.secondaryColor} 0%, #ffffff 100%);
        }

        [data-sonner-toast][data-type="success"] {
          --success-bg: ${activeBrand.secondaryColor};
          --success-border: ${activeBrand.primaryColor};
          --success-text: ${activeBrand.primaryColor};
        }
      ...
      document.head.appendChild(style);
    } else if (activeBrand.name === "Everk Cosméticos") {
      // Aplicar estilos específicos para Everk Cosméticos
      const style = document.createElement("style");
      style.id = "everk-styles";

      style.textContent = `
        /* Complete Sonner Toast System for Everk Cosméticos */
        :where(html[dir="ltr"]),:where([data-sonner-toaster][dir="ltr"]){--toast-icon-margin-start: -3px;--toast-icon-margin-end: 4px;--toast-svg-margin-start: -1px;--toast-svg-margin-end: 0px;--toast-button-margin-start: auto;--toast-button-margin-end: 0;--toast-close-button-start: 0;--toast-close-button-end: unset;--toast-close-button-transform: translate(-35%, -35%)}
        ...
        [data-sonner-toast][data-swipe-out=true][data-y-position=bottom],[data-sonner-toast][data-swipe-out=true][data-y-position=top]{animation-duration:.2s;animation-timing-function:ease-out;animation-fill-mode:forwards}
        ...
        @keyframes swipe-out-left{0%{transform:var(--y) translate(var(--swipe-amount-x));opacity:1}to{transform:var(--y) translate(calc(var(--swipe-amount-x) - 100%));opacity:0}}
        @keyframes swipe-out-right{0%{transform:var(--y) translate(var(--swipe-amount-x));opacity:1}to{transform:var(--y) translate(calc(var(--swipe-amount-x) + 100%));opacity:0}}
        @keyframes swipe-out-up{0%{transform:var(--y) translateY(var(--swipe-amount-y));opacity:1}to{transform:var(--y) translateY(calc(var(--swipe-amount-y) - 100%));opacity:0}}
        @keyframes swipe-out-down{0%{transform:var(--y) translateY(var(--swipe-amount-y));opacity:1}to{transform:var(--y) translateY(calc(var(--swipe-amount-y) + 100%));opacity:0}}
        ...
        /* Everk Brand Theme */
        .dynamic-gradient {
          background: linear-gradient(135deg, ${activeBrand.primaryColor} 0%, #5d06b3 100%);
        }

        @supports (color: color-mix(in lab, red, red)) {
          .dynamic-gradient {
            background: linear-gradient(135deg, ${activeBrand.primaryColor} 0%, color-mix(in srgb, ${activeBrand.primaryColor} 85%, black) 100%);
          }
        }
      `;
